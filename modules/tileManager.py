...
class TileManager():
    ...
    def changeSingle(self, row : int, col : int, TMpath : str):
        if TMpath.find("water") >= 0:
            ...
            self.quadTree.insert(self.GRID[row][col].Rect) if self.quadTree != None else print("No QuadTrees Detected")
    ...
    def loadTileLayout(self, tileLayout : str):
        ...
        for r in range(len(tileLayout)):
            for c in range(len(tileLayout[r])):
                ...
                self.MATRIX[r][c] = 1 if type == 0 or type == 4 else 0
                if type != 0 and type != 4:
                    self.quadTree.insert(self.GRID[r][c].Rect)
